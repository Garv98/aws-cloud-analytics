...
def lambda_handler(event, context):
    try:
        ...
        if not result_key:
            return _response(500, {"error": "Job is complete but resultKey is missing", "jobId": job_id})

        try:
            obj = s3_client.get_object(Bucket=report_bucket, Key=result_key)
            payload = json.loads(obj["Body"].read().decode("utf-8"))
        except ClientError as exc:
            return _response(500, {"error": "Failed to load result payload", "details": str(exc)})
        ...
        return _response(500, {"error": "Failed to load result", "details": str(exc)})
